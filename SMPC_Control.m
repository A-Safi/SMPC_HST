...
[PP,LL,GG]=dare(Ad,Bd,Q,R);
K=-inv(Bd'*PP*Bd+R)*Bd'*PP*Ad;

Q_bar=sdpvar(2*M,2*M);
cons=[Q_bar>=0,(-Q_bar+(Ad+Bd*K)'*Q_bar*(Ad+Bd*K)+(Q+K'*R*K))==0];
optimize(cons,0,sdpsettings('solver','sdpt3'));
Q_bars=value(Q_bar);
...
for i = 1:N_p
    objective = objective + (C*x{i}-r{i})'*Q*(C*x{i}-r{i}) + u{i}'*R*u{i};
    constraints = [constraints, x{i+1} == Ad*x{i} + Bd*u{i}+D*Ts];
    constraints = [constraints,-[1;1;1;1;1;1]*100e3<=u{i}<=[1;1;1;1;1;1]*100e3];
    constraints = [constraints,-[lk;lk;lk;lk;lk]-0.2<=x{i}(2:ny/2)<=[lk;lk;lk;lk;lk]+0.1];
end
objective = objective + (C*x{N_p+1}-r{N_p+1})'*Q_bars*(C*x{N_p+1}-r{N_p+1});
parameters_in = {x{1},[r{:}]};
solutions_out = {[u{:}]};
